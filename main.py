...
users = json_tran_list(read_data("data/users.json"))
message_data = json_tran_list(read_data("data/posts_message.json"))
posts_data = json_tran_list(read_data("data/posts.json"))
    ...
    if request.method == 'POST':
        ...
        for user in users:
            if user['username'] == username:
                flash('用户%s已经存在' % username, category='info')
                break
        else:
...
def posts():
    if request.method == 'POST':  # 判断是否是 POST 请求
        ...
        message_data.append(dict(name=get_name, text=get_text, time=t))
    return render_template("posts.html", data=message_data)
...
if __name__ == '__main__':
    # 非阻塞循环 进度 0%
    # time.sleep(3)
    # change_data("data/users.json", users)
    app.run(debug=True)